...
def cds_list():
    ...
    query ='''select distinct cl.bbg_cds_ticker, bd.ticker, bd.issuer 
...
def fx_rates(start_date, end_date):
    query =f'''
                select aq.pricedate, aq.quote, REPLACE(REPLACE(security_name, '-', ''), ' XRATE', '') AS [fx_tag], ai.bloomberg_ticker  
                from sha2.rates.asset_quotes aq 
                inner join (select *  from sha2.rates.asset_list where asset_type = 'cross' and 
                bloomberg_ticker in ('USDCAD WMCO Curncy','EURUSD WMCO Curncy','EURCAD WMCO CURNCY','GBPCAD WMCO CURNCY','USDJPY WMCO CURNCY')) ai on ai.asset_id=aq.asset_id
                where aq.pricedate >='{start_date}' and aq.pricedate <='{end_date}' '''
    cursor.execute(query)
    cursor.fetchall()
    df = pd.read_sql(query, conn)
